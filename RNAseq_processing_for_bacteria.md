...
Finally, note that recently a different approach is gaining popularity in the analysis of eukaryotic RNA-seq data. It is a pseudo-alignment approach that avoids the actual full mapping of reads to the genome and instead checks for compatibility of k-mers in a read with k-mers in known transcripts. Although the method has been developed mostly to deal with issues of carrying out differential expression at the gene level in the case of genes producing multiple transcripts, there is no obvious reason why the method couldn't be applied to bacterial RNA-seq data. In fact, because it is a lot faster than normal mapping methods, it may be useful in the case of very big datasets. This approach will be covered in a different walkthrough.

### IMPORTANT
Do not use this walkthrough blindly. Every command in every script should be checked. You need to ensure you understand what each step does (by looking up the relevant documentation) because a) as program versions change, there could be commands that no longer run or that need modifications and b) I may have made genuine mistakes when writing this guide.

## Preliminary note on running jobs on the server *icarus*
This walkthrough can be followed if you have access to our server *icarus*. If you don't, it may still be useful as a series of steps that you can follow on another server or even a laptop, if the software has been installed and your memory and hard disk allow you to complete the pipeline.

When running programs on *icarus*, I generally use a bash shell script that will run the same job for multiple samples. BEFORE I run the script that will work on multiple samples, I always run it once on a single sample to make sure it works as intended. There is no queueing system on icarus so the jobs will run sequentially as long as there free CPUs available. If you run the script directly on the command line, it will hang once you exit the shell or you are logged out, even if you put the job in the background. This is why some of the commands below you make use of `nohup` to ensure that the job will stay in the background after the shell has exited, e.g. the command:
```
...
Until recently, we used **fastqc** for quality control, followed by **multiqc** for easy summarisation across multiple samples. A version of *fastqc* that you can use on icarus is installed here: `/d/in7/s/fastqc_0.11.3`
This is quite likely not the latest version (but we can update it as and when needed).
...
```
where the run_fastqc.sh script in my case contains the following (note where fastqc is installed under /d/in7/s):
...
In the past, we have generally used *Trimmomatic* for trimming adapters and *fqtrim* for trimming poly(A) tails.

We have a version of fqtrim here: `/d/in7/s/fqtrim/fqtrim-0.94/fqtrim`
...
Very recently, a program called *fastp* was published. This is supposed to be the best and fastest program available currently for pre-processing of fastq data. I have not run it yet, but it is certainly worth taking a look at it. 
...
Once the reads have been trimmed, we map them to the genome. This generally requires an indexed version of the genome you want to map to, which you have to create following the guidelines of the mapping program. In the past we used *TopHat* which then was replaced by *Hisat2* for eukaryotes. *STAR* is also a popular alternative. I’ve used **_bowtie2_** and  _**bwa**_ for bacteria but I’d be more keen now to use the **_Rsubread_** software available from Bioconductor in R, as it is supposed to be fast and accurate. I was trying to find out if people use it for bacteria but most people still use bwa or bowtie2 or star. However, I don’t see any reason why Rsubread's *align* function shouldn't be used for bacteria. It also has the advantage that the mapping step can be done within R, together with the rest of your pipeline.

We have versions of hisat2 and bwa installed here (if you wish to try them):
```
/d/in7/s/HISAT2_v2.1
```
and here:
...
```
/d/in7/s/bwa-0.7.17/bwa index NC_003888.fa
```
where NC_003888.fa is a genome fasta file downloaded from NCBI and saved in the *genome* directory.
Once the genome is indexed, we can use bwa-aln (used here because the reads were shorter than 70 nucleotides) in a bash script as follows (don't forget bwa-aln should be used on the **preprocessed** fastq files, not the original fastq)
```bash
#!/bin/bash
...
The most interesting scripts are probably the ones that show you mapping statistics, calculation of TIN values (a measure of RNA degradation) and calculation of gene body coverage (which shows biases in the coverage of genes, usually at the 5’ or 3’ ends).
